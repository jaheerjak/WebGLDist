...
public class SwipeControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonUp(0) && canCheckSwipe)                     //if we lift mouse click and canCheckSwipe is true
        {
            ...
            float directY = -direction.y * throwForce / 1.7f;
            Debug.Log("directY "+directY);
            if (directY > 1250f) directY = 1250f;//add force to the ball rigidbody
            BallObj.GetComponent<Rigidbody>().AddForce(direction.x * throwForce, directY, throwForceZ / totalTime);
