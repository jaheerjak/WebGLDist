...
public class LevelManager : MonoBehaviour
{
    ...
    public void SpawnBall()                                        
    {
        if (availableBalls > 0)                                                 //we check if availableBalls is more than zero
        {
            ...
            UIManager.instance.BallText.text = "x"+availableBalls.ToString();       //update the text
        }
    ...
    {   //we spawn the level prefab at required position and get the LevelData component attached on it
        LevelData levelData = Instantiate(levelDatas[GameManager.instance.CurrentLevel].gameObject, new Vector3(0, 0.5f, -9f), 
        ...
        UIManager.instance.BallText.text = "x" + availableBalls.ToString();           //set the Balls text
